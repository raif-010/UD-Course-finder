...
export default function BottomNav({ activeTab, onChangeTab, favoritesCount, theme = 'dark' }: BottomNavProps) {
  ...
  return (
    <nav 
      className={`fixed bottom-0 left-0 right-0 z-50 py-2 pb-3 px-4 transition-all duration-300 ${
        isDark 
          ? "bg-[#0D1325]/95 backdrop-blur-md border-t border-[rgba(255,255,255,0.08)] shadow-[0_-10px_30px_rgba(5,8,22,0.6)]" 
          : "bg-white/95 backdrop-blur-md border-t border-slate-200 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]"
      }`}
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <div 
                  className={`absolute bottom-[-10px] w-8 h-[3px] rounded-t-full transition-all duration-300 ${
                    isDark
                      ? "bg-gradient-to-r from-[#6D3BFF] to-[#A855F7] shadow-[0_-4px_12px_rgba(109,59,255,0.6)]"
                      : "bg-gradient-to-r from-purple-600 to-indigo-600 shadow-[0_-4px_12px_rgba(147,51,234,0.4)]"
                  }`}
              ...
              <div className="relative" id={`nav-icon-container-${tab.id}`}>
                ...
                {tab.badge !== undefined && tab.badge > 0 && (
                  <span 
                    className={`absolute -top-1.5 -right-2 text-white text-[9px] font-extrabold w-4.5 h-4.5 rounded-full flex items-center justify-center animate-bounce duration-1000 border ${
                      isDark ? "bg-rose-500 border-slate-900" : "bg-red-500 border-white"
                    }`}
                    id={`nav-badge-${tab.id}`}
                  >
                    {tab.badge}
              ...
              <span 
                className={`text-[10px] font-bold tracking-wider mt-1 transition-all duration-200 ${
                  isActive 
                    ? isDark ? 'text-white' : 'text-purple-650'
                    : isDark ? 'text-[#A0AEC0] group-hover:text-white/80' : 'text-slate-400 group-hover:text-slate-700'
                }`}
                id={`nav-label-${tab.id}`}
              >
                {tab.label}
